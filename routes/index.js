...
/*
This endpoint allows for a rendering of the SPA data.
This means http://localhost:3000/view/talks/tags/patterns will display the page http://localhost:3000/
but still retain the url to be parsed by the clients javascript file.
*/
...
router.get("/talks/speaker/:term", function(req, res) {
  ...
  var selectedtalks = talks.filter(function(talk) {
    return (
      talk.speaker.toLowerCase().search(req.params["term"].toLowerCase()) >= 0
    );
  });
  res.status(200);
...
router.get("/talks/description/:term", function(req, res) {
  ...
  var selectedtalks = talks.filter(function(talk) {
    return (
      talk.description.toLowerCase().search(req.params["term"].toLowerCase()) >=
      0
    );
  });
  res.status(200);
